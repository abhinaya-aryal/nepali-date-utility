import { NepaliDate, GregorianDate } from "../types";
import { bsCalendarData } from "../data/bs-calendar";

const bsEpoch: NepaliDate = { year: 2000, month: 1, day: 1 };
...
export function convertADToBS(adDate: Date): NepaliDate {
  ...
  while (remainingDays > 0) {
    ...
    if (!bsCalendarData[bsYear]) {
      throw new Error(`Year ${bsYear} is outside the supported range (2000-2090)`);
    }

    const daysInMonth = bsCalendarData[bsYear][bsMonth - 1];
    if (typeof daysInMonth !== 'number') {
      throw new Error(`Invalid calendar data for year ${bsYear}, month ${bsMonth}`);
    }
...
export function convertBSToAD(bsDate: NepaliDate): GregorianDate {
  let bsYear = bsEpoch.year;
  let bsMonth = bsEpoch.month;
  let bsDay = bsEpoch.day;
...
export function compareDates(a: NepaliDate, b: NepaliDate): number {
...
export function getFirstDayOfMonth(bsYear: number, bsMonth: number): number {
  ...
  const bsDate: NepaliDate = { year: bsYear, month: bsMonth, day: 1 };
...
export function addMonths(date: NepaliDate, months: number): NepaliDate {
